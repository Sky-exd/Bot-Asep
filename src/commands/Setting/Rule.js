  ...
  .addSubcommand((subcommand) =>
    subcommand
      ...
      .addChannelOption((option) =>
        option
          .setName('channel')
          .setDescription('Channel yang ingin ditargetkan untuk menampilkan aturan')
          .setRequired(true),
      ),
  );
...
export const run = async ({ interaction }) => {
  ...
  switch (subcommand) {
    case 'add': {
      ...
      if (!hasAdminRole) {
        return interaction.editReply({ content: 'Kamu tidak memiliki izin untuk menggunakan perintah ini.', ephemeral: true });
      }

      try {
        const rule = new Rule({ rule: ruleText, banner: bannerUrl });
        await rule.save();
        await interaction.editReply('Aturan baru telah ditambahkan!' + (bannerUrl ? ' Dengan banner.' : ''));
      } catch (error) {
        console.error(`Error in ${__filename}\n`, error);
        await interaction.editReply({ content: 'Terjadi kesalahan saat menambahkan aturan. Silakan coba lagi nanti.', ephemeral: true });
      }
      break;
    }
    case 'remove': {
      ...
      try {
        ...
        if (ruleNumber < 1 || ruleNumber > rules.length) {
          return interaction.editReply({ content: 'Nomor aturan tidak valid.', ephemeral: true });
        }

        const ruleToRemove = rules[ruleNumber - 1];
        await Rule.deleteOne({ _id: ruleToRemove._id });
        await interaction.editReply(`Aturan nomor ${ruleNumber} telah dihapus!`);
      } catch (error) {
        console.error(`Error in ${__filename}\n`, error);
        await interaction.editReply({ content: 'Terjadi kesalahan saat menghapus aturan. Silakan coba lagi nanti.', ephemeral: true });
      }
      break;
    }
    case 'list': {
      ...
      try {
        ...
        await interaction.editReply(`Daftar peraturan telah dikirim ke ${targetChannel}`);
      } catch (error) {
        console.error(`Error in ${__filename}\n`, error);
        await interaction.editReply({ content: 'Terjadi kesalahan saat mengambil daftar peraturan. Silakan coba lagi nanti.', ephemeral: true });
      }
      break;
    }
    default:
