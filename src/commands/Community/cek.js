...
export const run = async ({ interaction }) => {
  ...
  switch (subcommand) {
    ...
    case "role": {
      ...
      if (!user) return await interaction.editReply("Tolong tag pengguna yang ingin dicek rolenya.");
      const memberGuild = interaction.guild.members.cache.get(user.id)
      const role = memberGuild.roles.cache.filter((role) => role.name !== "@everyone").map((role) => roleMention(role.id)).join(",  ");
      await interaction.editReply({
        embeds: [embedBase({
          type: "info",
